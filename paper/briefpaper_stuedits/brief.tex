...
\caption{\textbf{C.} Cortical whisker barrels from \cite{zheng_signal_2001}
  with annotations for approximate axes and lines drawn to join the
  interstices into a putative Voronoi diagram.  \textbf{B.}  Top: Stochastic
  initial condition for $a_i$ with $i=13$ (barrel C4). The roll-off towards
  the boundary (thin line) ensures the boundary conditions are
  satisfied. Bottom: Guidance molecules in the simulated cortex. Molecule 1 is
  expressed high to low in an approximately anterior to posterior
  gradient. Molecule 2 is expressed lateral to medial (analogously to ephrin -
  Vanderhaegen et al) \textbf{C.}  Barreloid centres for mystacial whiskers
  taken from \cite{haidarliu_size_2001} are shown as black circles. The dashed
  grey line shows the boundary of the barreloid field. It is assumed that
  there are two linearly increasing, orthogonal signalling molecules in the
  thalamus, such that the location of each barreloid within the graded
  thalamic signal field can differentiate the efferent axons, with the result
  that axons from each barreloid interact to varying degrees with the cortical
  signalling gradients shown in B. Coloured arrows show, for each barreloid,
  the magnitude (and sign) of the interaction that its thalamocortical
  projection will have with each signalling gradient in the cortex. The
  mappings from barreloid location to interaction strength are
  linear. \textbf{D.}  Barrel cortex simulation. The maps indicate which
  thalomocortical axons make maximum connection density. Each hex element is
  coloured one of 41 \cmnt{FIXME: Got to here} unique colours depending on which thalamocortical axon
  makes the most connections in that element. The mapping between barrel
  identity and colour is given in F.  \textbf{G.} The Honda Dirichlet measure
  as a function of simluation time for the simulations show in
  D-F. \textbf{H.}  Guidance molecules in the simulated Fgf8 misexpression
