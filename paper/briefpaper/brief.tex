...
% Red for Stuart's first updates
...
% Justify gamma/interaction parameter choice
...
\begin{figure}
  \begin{fullwidth}
    ...
    \caption{\paramsearch{\textbf{A} Exploring the parameter space of the model given
...
\paramsearch{To determine the robustness of the model to its parameters, we
  ...
  shows the results for a separate metric. Each column of maps shows results
  for a different value of the ratio $\alpha / \beta$. The diffusion
  parameter, $D$, varies with the $x$ axes and the strength of competition,
  $\epsilon$ varies with the $y$ axes. The colour maps are chosen so that
  yellow indicates desirable properties for the simulations: Lower values of
  $\delta$ indicate good Voroni patterns and are yellow; Lower values of
  $\eta$ indicate good pattern matching; Higher values of $\omega$ indicate
  greater selectivity and are also coloured yellow. White squares indicate
  that the simulation was not stable for that parameter set.}

\paramsearch{The maps indicate that the pattern is not strongly influenced by the
