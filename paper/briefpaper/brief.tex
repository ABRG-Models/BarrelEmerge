...
\begin{center}
  ...
  \captionof{figure}{
    \MPthreePred{
      Two simulated whisker barrel manipulations.  Guidance fields
      ...
      Fig.\,\ref{fig:main}C. \textbf{C} Simulating whisker trimming by a
      reduction of the competition parameter, $\epsilon$, in the expression
      $\chi_i$ (Eq.\,\ref{eq:comp}) for the C3 thalamocortical projection. For
      C3, $\epsilon$ is replaced by $m\,\epsilon$, where $m$ is a scalar
      ...
      Fig.\,\ref{fig:main}C. The pattern formed after 30000 steps with
      $m=0.64$. Barrel C3 is reduced in size as its axon branching competes
      less strongly with its neighbours, whose area is slightly
      increased. \textbf{D} Individual barrel area plotted vs.~$m$ for C3
      (black squares) shows that
      as $m$ is reduced, its area goes down, disappearing
      for $m\approx0.5$, whereas the mean area of neighbouring barrels
      B3, C2, C4, D2 \& D3
      (grey circles) increases. Scale bars are 1\,mm.
    }
  }
  \label{fig:fgf8}
\end{center}
