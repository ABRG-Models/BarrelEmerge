...
\caption{\textbf{A.} Cortical whisker barrels from \cite{zheng_signal_2001}
  ...
  ($\delta=0.022$). \textbf{B.}  Top: Stochastic initial condition for $a_i$
  with $i=1$ (barrel `$\alpha$'). The roll-off towards the boundary (thin line)
  ensures the boundary conditions are satisfied. Bottom: Guidance molecules
  $\rho_1(\mathbf{x})$ and $\rho_2(\mathbf{x})$ in the simulated
  cortex. $\rho_1(\mathbf{x})$ varies along the anterior-posterior axis;
  $\rho_2(\mathbf{x})$ is expressed along the lateral/medial axis, analogous
  to ephrin A5 \citep{vanderhaeghen_mapping_2000}. \textbf{C.}  Barreloid
  centres for mystacial whiskers taken from \cite{haidarliu_size_2001} are
  shown as black circles. The dashed grey line shows the boundary of the
  barreloid field. It is assumed that there are two linearly increasing,
  orthogonal signalling molecules in the thalamus, such that the location of
  each barreloid within the graded thalamic signal field can differentiate the
  efferent axons, with the result that axons from each barreloid interact to
  varying degrees with the cortical signalling gradients $\rho_1$ and
  $\rho_2$. Blue (green) arrows show, for each barreloid, the magnitude (and
  sign) of the interaction that its thalamocortical projection will have with
  $\rho_1$ ($\rho_2$). The mappings from barreloid location to interaction
  strength are linear. \textbf{D.}  Barrel cortex simulation with parameters
  $\alpha=3$, $\beta=20$, $k=3$, $D=0.2$, $\gamma\in\pm 2$ and
  $\delta{t}=0.0001$. Colours indicate $\arg\max_i(c_i)$ for 41 TC
  projections.  \textbf{E.}  The Honda Voronoi measure, $\delta$, (red) and a
  pattern fitting metric based on the distance, $d_i$, between simulated and
  experimental barrel centroids and the absolute difference in their areas,
  $\Delta A_i$ given by $\sigma = \sum_i^N d_i^2 \times \sum_i^N \Delta A_i$
  (black) plotted as a function of time for the simulations shown in
  D. \textbf{F.}  Guidance molecules in the simulated Fgf8 misexpression
  experiment. $\rho_1$ is mirrored over the anterior-posterior axis. The
  simulation indicates that the mirroring of the barrel field can occur as a
  result of the mirroring of guidance molecule 1.}
\label{fig:main}
\end{figure*}
