...
\cmnt{We performed a sensitivity analysis to determine if the quality of the
  pattern was sensitive to any of the following inputs: i)~The angle and
  magnitude of each of the two guidance fields (Fig.\,\ref{fig:main}A);
  ii)~the magnitude, offset and characteristic length scale of the noise
  applied to $a_i$ at $t=0$; and iii)~the magnitude of each of the interaction
  parameters, $\gamma_{i,j}$. We analysed the effect on the three pattern
  metrics, $\delta$, $\eta$ and $\omega$ evaluated after 30000 simulation
  steps. First, we conducted 10 runs of the simulation to evaluate the mean
  and standard deviation (as a result of the different random initial
  conditions for $a_i$) of the three metrics in the default model
  (Fig.\,\ref{fig:main}C--E) which were $\delta=0.089\pm{0.004}$,
  $\eta=0.531\pm{0.005}$ and $\omega=0.2165\pm{0.0001}$). These results are
  not strongly affected by the random initial conditions for $a_i$ but give a
  baseline from which to evaluate whether changes in the other inputs have a
