...
\begin{equation} \label{eq:comp}
\color{blue}
\chi_i(\mb{x}, t) = \frac{a_i F}{N-1} \nabla \hat{a}_i,
\color{black}
\end{equation}
%
\cmnt{which contributes to the \emph{flux of axonal branching} by making
  branching density for each projection `flow away' from regions} where the
branches of other projections are dense. \cmnt{$F$ parameterizes the strength
  of this competitive interaction.} Note that this operation is local to
individual afferent projections.
