  ...
  \begin{fullwidth}
    ...
    \caption{\textbf{A} Left shows a cytochrome oxidase stain obtained from rat S1
      ...
      \textbf{E} \cmnt{The localization measure plotted across the domain
        ...
        by $t=50000$ is largely absent at $t=1000$. The
        colour map
...
within \cmnt{$\approx 10000$} iterations (Fig.\,\ref{fig:main}D). A measure of the difference
in shape between real and simulated barrels revealed a strong correspondence
(see Fig.\,\ref{fig:main}D).

% Full page width
\begin{figure}
  \begin{fullwidth}
    \includegraphics[width=\linewidth]{./paramsweep.png}
    \caption{\cmnt{\textbf{A} Parameter sweep caption goes here}}
    \label{fig:paramsweep}
  \end{fullwidth}
\end{figure}

\cmnt{Parameter sweep paragraph goes here.}
...
thalamic projection (Fig.\,\ref{fig:main}E). Together these results suggest that by the
...
(Eq.\,\ref{eq:comp}), and iii) \cmnt{REVIEW: plotting $a(t)$, $c(t)$ and
  $a(t)-c(t)$ showed that there was some leaking of $a$ going on at the
  boundary; increasing the boundary fall off distance to 3 times hex to hex
  distance reduces the leaking to a negligible amount. So the branchdesnity of
each projections IS conserved over time.} the branch density of
each projection is conserved over time.
