...
\cmnt{I've left this section in black, although it is all new, cf the PNAS
  draft.}
...
%
% computing the flux using a finite difference method,
%
% numerical integration by the fourth-order Runge-Kutta method,

In these simulations, the cortex was modelled as a two dimensional lattice in
which each element represented a small area of the cortical sheet. We chose to
use hexagonal elements to simplify the computation of the 2D Laplacian, a
spatial operator used in reaction diffusion systems. Within a boundary traced
from rat primary barrel cortex (Fig.\,1A) we set the hex-to-hex distance
...
variables: 41 branching densities ($a_i$) and 41 connection densities
($c_i$). Each hex also contained 82 time-independent variables; the $x$ and
$y$ components of the sum of the product of the interaction paramters with the
two molecular signalling gradients.

The rates of change of each of the time-dependent variables (Eqs.\,\ref{eq:dc}
\& \ref{eq:da}) was computed using a fourth-order Runge-Kutta method. The most
involved part of this computation was finding the divergence of the flux of
axonal branching, $\mb{J}_i(\mb{x},t)$---the term in parentheses in
Eq.\,\ref{eq:da}:
%
\begin{equation}
  \label{eq:divJ}
...
and then apply vector calculus identities to compute this term. Because
the divergence operator is distributive, Eq.~\ref{eq:divJ} can be expanded
(dropping all explicit references to space or time dependence):
%
\begin{equation}
\nabla\vcdot\mb{J}_i = \nabla\vcdot\big(D \nabla a_i\big) - \nabla\vcdot\big(a_i \mb{g}_i\big)
\end{equation}
%
Because $D$ is a constant and by applying the vector calculus product rule
identity to $\nabla\vcdot\big(a_i \mb{g}_i\big)$ we obtain:
%
%%\begin{equation}
...
following \cite{lee_hexagonal_2014}. The computation of the mean value of the
Laplacian across the area of one hexagon of area $\Omega$ located at position
$\mb{p}_0$, with neighbours at positions $\mb{p}_1$--$\mb{p}_6$ is:
%
...
The computation of the second term in Eq.\,\ref{eq:divJExpanded},
...
%
\Rightarrow \langle a_i(\mb{p}_0,t)\nabla\vcdot\mb{g}(\mb{p}_0)\rangle & = \frac{a_i(\mb{p}_0,t)}{3d} \bigg( \sum_{j=1}^{6} \big(\mb{g}_j^x + \mb{g}_0^x\big) \cos (60\times(j-1)) + \sum_{j=1}^{6} \big({\mb{g}_j^y + \mb{g}_0^y}\big) \sin (60\times(j-1)) \bigg) \\
%
\end{split}
\end{equation}

The final expression is suitable for a numerical computation and can
be found in \code{compute\_divJ()} as \code{term2}.

The final term in Eq.~\ref{eq:divJExpanded} is the scalar product of two
vector fields which is straighforward to compute from their Cartesian
components.

The advantage of separating this computation out is that we can ensure that,
at the boundary, $\mb{J}$ resulting from the first term remains 0, by the
ghost cell method, then $\mb{g}_i(\mb{x})$ can be tailored so that it, and its
...
$\mb{J}$. This is achieved by applying a fairly sharp logistic function based
on distance to the boundary to $\mb{g}(\mb{x})$.

All code required to reproduce these results is available at
\url{https://github.com/ABRG-Models/BarrelEmerge}
