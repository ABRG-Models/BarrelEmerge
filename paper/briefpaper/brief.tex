...
\begin{figure}
  \begin{fullwidth}
    ...
    \caption{\textbf{A} Left shows a cytochrome oxidase stain obtained from rat S1
      ...
      S1). \textbf{D} Red dots show the Honda-$\delta$ metric obtained from \stu{the}
      simulation approaching that obtained from \cmnt{real} barrels in \textbf{A} (dotted
      line); black squares \stu{provide a} measure \cmnt{of} the correspondence
      between the real and simulated barrel shapes, $\eta$ (the sum of differences in
      area; units \cmnt{mm$^2$});
      \cmnt{grey hexagons \stu{provide a measure of how selectively each cortical site is innervated}, $\omega = \oiint_{S} \left( \mathrm{max}_i(c_i)/\sum_{i=1}^{N} c_i\right) \mathrm{d}S$. }
      \textbf{E} \stu{Plotted across the cortical sheet, the selectivity develops to reveal an alignment with the emergent barrel boundary shapes}
      \cmnt{The colour map indicates the values of  $\mathrm{max}_i(c_i)/\sum_{i=1}^{N} c_i$. Overlaid
        contours show $c_i > 0.95 \mathrm{max}_i(c_i)$.} All scale bars 1\,mm.}
    \label{fig:main}
  \end{fullwidth}
\end{figure}
...
i.e., the putative barreloid center. \stu{As such, we consider linear gradients aligned to anterior-posterior and medial-lateral axes through the VPM, and as $\gamma$-values use the coordinates of the barreloid centers in these axes as they appear in \citep{haidarliu_size_2001} (Fig.\,\ref{fig:main}B).} \stu{Complementary thalamic and cortical molecular guidance gradients of this form have been well established experimentally \citep{vanderhaeghen_mapping_2000,miller_epha7-ephrin-a5_2006}, e.g., cells originating in the medial part of ventrobasal thalamus (comprising VPM and VPL) express high levels of EphA receptors and project to the lateral part of S1, which expresses low levels of ephrin-A5, and cells originating in the lateral part of ventrobasal thalamus express low levels of EphA receptors and project to the medial part of S1, which expresses high levels of ephrin-A5 (see Gao, 1998; Dufour et al., 2003; Vanderhaegen \& Polleux, 2004; Speer \& Chapman, 2005)}
...
gradients aligned with the anterior-posterior and medial-lateral axes. From
...
Fig.\,\ref{fig:main}D, squares). \cmnt{In a chosen region, the ratio of
  ...
  of the localization of the connection density. We computed this measure
  across the entire barrel domain and named it $\omega$ (see
  Fig.\,\ref{fig:main}D, grey hexagons). $\omega$ increases during the first
  20000 simulation steps and converges on a stable
  value. Fig.\,\ref{fig:main}E shows the localization measure plotted as a
...
\begin{figure}
  \begin{fullwidth}
    ...
    \caption{\cmnt{\textbf{A} Exploring the parameter space of the model given
        ...
        localization (separation) of the connection densities for the 41
...
\cmnt{The maps indicate that the pattern is not strongly influenced by the
  ...
  trade-off between good localization and good Voroni pattern
  conformation. The
...
\begin{figure}
  \begin{fullwidth}
    ...
    \caption{\cmnt{Sensitivity analysis. Column \textbf{A} shows...}}
    \label{fig:sens}
  \end{fullwidth}
\end{figure}
...
%
  ...
  $\omega$ (which measures localization of connections, $c_i$) after running
...
\cmnt{First, we conducted 10 runs of the simulation shown in
  ...
  conditions for $a_i$) were $\delta=0.089\,(0.004)$, $\eta=113.5\,(1.2)$ and
...
\cmnt{Next, we modified the parameters of the generating uniform distribution
  ...
  $\eta=115\,(1.9)$ and $\omega=0.2166\,(0.0003)$. Paired two-sample t-tests
  ...
  $\eta=114.31$ and $\omega=0.21649$) and that stochasticity in
...
% eta     114.31       113.5(1.2)      <1                   115 (1.9)        <1
...
\cmnt{Fig\,\ref{fig:sens}A shows the results for perturbations ii) to iv). The
  ...
  and the localization of the fields are not strongly affected by interaction
...
\cmnt{Add a description of $\eta$, the pattern-matching measure, here.}
