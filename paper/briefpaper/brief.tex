...
\cmnt{which contributes to the \emph{flux of axonal branching} as an
  ...
  projection diffuse away from regions} where the branches of other projections
...
i.e., the putative barreloid center. \cmnt{Thus, we suggest that there exist a
  pair of signalling molecules whose expression varies linearly (with
  orthogonal gradients) across the thalamus, analgous to the ephrins, whose
  existence is well known in the cortex. Thalamocortical axons growing from a
  region in the thalamus with high expression of one of these molecules will
  interact with a corresponding cortical signalling molecule more strongly
  than an axon emanating from a region with low expression of the thalamic
  signal.}  \cmnt{We defined two orthogonal, linear gradients oriented in the
  anterior-posterior and medial-lateral directions and used the coordinates of
  41 barreloid centers recreated from \cite{haidarliu_size_2001},
  corresponding to the macrovibrissae, to populate 41 pairs of $\gamma$
  values.}  (Fig.\,\ref{fig:main}B). \cmnt{In other words, we simply scaled
  the barreloid coordinates in an anterior-posterior/medial-lateral coordinate
  system to create the $\gamma$ values in the range [-2,2]. Note that positive
  \emph{and} negative $\gamma$ values were generated; the origin of the
  coordinate system was located at the approximate center of the thalamic
  barreloid field. This prevented an overall drift of the cortical pattern
  away from the centroid of the barrel field and may explain why
  coordinate-giving transcription factors and the ephrins are often found as
  pairs acting in opposing directions.}  \cmnt{We concentrated on
  macrovibrissae (excluding microvibrissae) because it is easy to identify
  their corresponding structures both in the thalamus and in the cortex. We
  also excluded the 5 rhinal whiskers because their cortical representation is
  in a separate island, distinct, and somewhat removed from the representation
  of the macrovibirissae.}
...
%
  iii)~random noise (of various length scales) applied to the guidance fields;
%
  and
%
  ...
  Fig.\,\ref{fig:main}A).
%
  We analysed the effect on the three pattern metrics, $\delta$ (the Honda
  Voronoi metric), $\eta$ (the pattern-match metric) and $\omega$ (which
  measures localization of connections, $c_i$) evaluated after 30000
  simulation steps.}
...
\cmnt{First, we conducted 10 runs of the simulation shown in
  ...
  three metrics in the default model. The results (where deviations occur as a
  result of the different random initial conditions for $a_i$) were
  $\delta=0.089\pm{0.004}$, $\eta=113.5\pm{1.2}$ and
  $\omega=0.2165\pm{0.0001}$. None of the three metrics were strongly affected
  by the random initial conditions for $a_i$. The results give a baseline from
  which to evaluate whether changes in the other inputs have a statistically
  significant effect.}

\cmnt{Next, we changed the initial condition for $a_i$, changing the
  parameters of the generating uniform distribution to set each hex to
  $0.4+{U}(0,0.4)$. We ran this system 10 times and obtain the result
  $\delta=0.081\pm{0.004}$, $\eta=110\pm{1}$ and $\omega=0.2284\pm{0.0001}$. A
  t-test demonstrates that this is ????. A final test of a noise-free initial
  condition with $a_i=0.2$ for every hex also ????.}
