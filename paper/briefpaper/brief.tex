...
dynamics are quite unline classic chemospecificity models
...
bounded on each iteration to the initial (random) density of unconnected
  branches at $t=0$
%
...
Eqs.~\ref{eq:dc}--\ref{eq:comp} at time $t-{\delta}t$. Note that this
...
\citealp{haidarliu_size_2001} (Fig.\,1B).
...
(Eqs.~\ref{eq:dc}--\ref{eq:da}), ii) at each We speculate that a projection interacts with a given ephrin field in
...
centerlocation the branching rate of a
...
The emergence of barrels in simulation required cWe speculate that a projection interacts with a given ephrin field in
the cortical subplate with a strength determined by the concentration of a
similar molecule at its thalamic origin, i.e., the putative barreloid
centerompetition between thalamic
projections in terms of synaptic connectivity and an implicit self/other
identifier amoungst projections, and also competition in terms of cortical
space (as represented by $\chi$). This latter requirement may account for the
absence of
...
\begin{split}
\langle D \dvrg a_i(\mb{p}_0,t) \rangle  = \frac{1}{\Omega} \oiint_\Omega \dvrg a_i(\mb{x}, t) \dif\Omega & = \frac{1}{\Omega} \oint \frac{\partial a_i}{\partial \hat{\mb{n}}} d\gamma \\
...
\end{equation}
%
where $v = d/\sqrt{3}$ is the length of each edge of the hexagon and $d\gamma$
...
\begin{split}
%
\frac{1}{\Omega} \oiint_\Omega a_i\nabla\vcdot\mb{g}\;\dif\Omega & = \frac{a_i(\mb{p}_0,t)}{\Omega}  \oint \mb{g}\vcdot d\hat{\mathbf{n}} \\
%
...
\end{equation}
%
Both this last expression, and the final expression of Eq.\,\ref{eq:lapl} can
