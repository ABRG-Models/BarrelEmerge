...
\MPone{For simplicity, the two simulated thalamic interaction gradients, as
  ...
  estimated from Fig 5d of} \citep{haidarliu_size_2001}, \MPone{and scaled
...
\cmnt{Across a wide range of parameter values, random initial conditions (a
  ...
  organised thalamocortical projections, confirming that barrel maps can
  self-organise in the absence of pre-specified center points. The
...
\citealp{senft_mouse_1991}) \metrics{can emerge within $\approx$~20000
  ...
  to the simulation was obtained (see \emph{Materials and Methods} for
  ...
  barrel map self-organised in the example simulation (Fig.\,\ref{fig:main}D,
...
\MPthreePar{The parameters of the example simulation are indicated in
  ...
  identifiable barrel fields (Fig.\,\ref{fig:paramsweep}Biii).}
...
\MPthreeSens{Using the parameters of the example simulation
  ...
  be switched (see example map Fig\,\ref{fig:sens}Bi). Adding noise to the
  ...
  at the domain boundaries (Fig\,\ref{fig:sens}Bii). Varying the slope of one
  ...
  axis (Fig\,\ref{fig:sens}Biii), while pattern formation was not strongly
  influenced by relaxing the assumption that the gradients of the two cortical
  guidance fields are orthogonal (Fig\,\ref{fig:sens}Biv). Overall, the
...
\citealp{assimacopoulos_fibroblast_2012}. \MPthreePred{We also assume that a
  ...
  barrel fields comprising $2N$ barrels (Fig\,\ref{fig:main}B), consistent
...
\begin{center}
  \includegraphics[width=\linewidth]{./Fig4.png} \captionof{figure}{
    \MPthreePred{Two simulated whisker barrel manipulations.  Guidance fields
      ...
      after 30000 steps with $m=0.64$. Barrel C3 is reduced in size as its
      axon branching competes weakly with its neighbours, whose area is
      slightly increased. \textbf{D} Individual barrel area (mm$^2$) plotted
      vs.~$m$ for C3 (black squares) shows that as $m$ is reduced, its area
      goes down, disappearing for $m\approx0.5$, whereas the mean area of
      neighbouring barrels B3, C2, C4, D2 \& D3 (grey circles) increases. The
      dotted grey line indicates $m=0.64$ for comparison with panel C.}}
  \label{fig:fgf8}
\end{center}
...
\MPthreePred{To investigate the response of the model to environmental
  ...
  \citep{kossut_effects_1992} (65\%) was obtained in simulation when the
