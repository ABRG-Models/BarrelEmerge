...
def mapplot_pub (F, ttarg, param_tuples, logdirbase, figrows, figcols, plotfirst):
    ...
    print ('plotfirst: {0}'.format(plotfirst))
    plotiter = 1+plotfirst
    for _pt in param_tuples:
        print ('tuple: {0}'.format(_pt))
        ep__ = _pt[0]
        ab__ = _pt[1]
...
#F_vals = [ 0.03, 0.08, 0.19, 0.48, 1.2, 3.0 ]
        ...
        try:
            ...
            sf.sbfs = 48
            lstr = 'unknown'
            if plotiter-plotfirst == 1:
                lstr = '(i)'
            elif plotiter-plotfirst == 2:
                lstr = '(ii)'
            elif plotiter-plotfirst == 3:
                lstr = '(iii)'
            sf.idlabel = lstr
            sf.drawid = False # To draw a box or something around the map in an ID colour
            sf.textid = True # To put in i) or ii) etc and the parameter values
            sf.showNames = True
            sf.domcentres = bdo.domcentres[0]
            col = sc.Colour()
            sf.boundarylw = 1.0
            ...
            # Need to pass in plotit
            print ('setFig with plotiter: {0}'.format(plotiter))
            sf.setFig (F, figrows, figcols, plotiter)
            sf.plotPoly()
...
# Second/Publication versino of paramplot()
def paramplot_pub (sdata, F, column_tag, x_tag, y_tag, ktarg, ttarg, param_tuples = [], col_trim_front = 0, col_trim_back = 0, cmapstr='viridis', logdirbase=''):
    ...
    #
    # Now plot the chosen plots
    #
    mapplot_pub (F, ttarg, param_tuples, logdirbase, graph_rows, len(colall), ((graph_rows-1)*len(colall)))
