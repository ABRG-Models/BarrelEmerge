...
logdirbase = "/home/seb/gdrive_usfd/data/BarrelEmerge/rel_guide_strengths_sq_UD"
...
for ud in np.linspace(0.1,0.9,3):

    logdirname = "{0}{1:.1f}".format (logdirbase, ud)
...
l3, = ax1.plot(uds, s_resids_h, 's-', label='Summed residuals to horz. line fits')
#l4, = ax1.plot(uds, gfits_v, '*-', label='inv. verticality (0:vertical)')
...
#plt.legend((l1, l2, l3, l4, l5), ('Honda Delta', 'Summed resid.', 'Summed resid (h).', 'inv. vert.', 'inv. horz.'), loc='right')
plt.legend((l1, l2, l3), ('Honda Delta', 'Summed resid (v)', 'Summed resid (h)'), loc='right')

F0.tight_layout()
F1.tight_layout()

F0.savefig ('rel_guide_maps.png')
F1.savefig ('rel_guide_graph.png')
