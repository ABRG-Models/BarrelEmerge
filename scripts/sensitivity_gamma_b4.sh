...
# gamma inputs. As there are 82 of them, this is not an exhaustive
# treatment. Rather, I will take three barrels across the domain and
# vary their gamma_i,js.
...
CONFIG_DIR="configs/rat/sensitivity_gamma_b4"
...
# Barrel B4
for b4_gamma_i in -0.6 -0.3 0 0.3 0.6; do
    for b4_gamma_j in -0.6 -0.3 0 0.3 0.6; do

        # B4 normal gamma is:[-0.5865031913897942, 0.6101032224741925]
        ((B4I=b4_gamma_i - 0.5865))
        ((B4J=b4_gamma_j + 0.6101))
        JSON="sa_gamma_b4_i${B4I}_j${B4J}.json"
        ...
        cat > ${CONFIG_DIR}/${JSON} <<EOF
{
    ...
    "logevery": 30000,
    "overwrite_logs": true,
    "logbase" : "/home/seb/gdrive_usfd/data/BarrelEmerge/sensitivity_gamma_b4/",
    ...
    "tc": [
        ...
        { "alpha" : ${ALPHA}, "beta" : ${BETA}, "xinit" : -0.16,   "yinit" : 0.0, "sigmainit" : 0.0, "gaininit" : 1.0, "gamma" : [${B4I}, ${B4J}], "name" : "B4" }, // B4
